const Estilo = require('../models/Estilo')(/* construtor */);
...
controller.listar = function(req, res) {
   ...
   Estilo.find().exec().then(
      // Callback do bem
      function(estilos) {
         // Retorna TODAS as estilos em um vetor,
         // ou um vetor vazio se não tiver nada
         res.json(estilos).end();         
      },
      // Callback do mal
      function(erro) {
         console.error(erro);
...
controller.obterUm = function(req, res) {
   ...
   Estilo.findById(id).exec().then(
      // Callback do bem
      function(estilo) { // Retorna 0 ou 1 veículo
         if(estilo) {  // Encontrou
            res.json(estilo).end();
         }
         else {   // Não encontrou
...
controller.atualizar = function(req, res) {
   ...
   Estilo.findOneAndUpdate({_id: id}, req.body).exec().then(
      // Callback do bem
      function(estilo) {
         if(estilo) {     // Encontrou e atualizou
...
controller.excluir = function(req, res) {
   ...
   Estilo.findOneAndDelete({_id: id}).exec().then(
      // Callback do bem
      function(estilo) {
         if(estilo) {     // Encontrou e excluiu
            res.sendStatus(204).end();
         }
